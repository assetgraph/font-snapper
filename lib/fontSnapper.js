...
function snapToAvailableFontProperties(fontFaceDeclarations, propsToSnap) {
  ...
  let [value, ...operations] = String(desiredWeight).split('+');
  if (value === 'lighter' || value === 'bolder') {
    // font-weight: lighter or font-weight: bolder can't really be resolved properly
    // without knowing the computed font-weight of the parent element, but assuming 400 is better than breaking:
