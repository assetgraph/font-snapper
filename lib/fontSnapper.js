...
function normalizeFontStretchToPercentage(fontStretchValue) {
...
function snapToAvailableFontProperties(fontFaceDeclarations, propsToSnap) {
  ...
  const fontStretchRangeByFamilyMatch = new Map(
    familyMatches.map(familyMatch => {
      let [min, max] = familyMatch['font-stretch']
        .split(/\s+/)
        .map(normalizeFontStretchToPercentage);
      max = max || min;
      return [familyMatch, { max, min }];
    })
  );

  const desiredFontStretchPercentage = normalizeFontStretchToPercentage(
  ...
  //  const familyMatchesInFontStretchRange = [];
  let bestDistance;
  let bestMatch;
  let stretchMatches = [];
